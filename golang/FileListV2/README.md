...
- TODO: Copy specific blobs to another Blob store (like Export/Import, Change Repository Blob Store)
...
#### List .properties which matches with `-pRx "{regex}"`, also including the properties content `-P` in the saving file `-s`, but only the first N `-n N`
```
