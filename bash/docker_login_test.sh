...
for _h in ${@:2}; do
  ...
  curl -D/dev/stderr -k -H "Accept: application/json" -L "${_h%/}/v2/"
  echo "# Generating token from host:${_h} with userId:${_userid} ..."
  # --get --data-urlencode "service=${_h%/}/v2/token"
  _TOKEN="$(curl -D/dev/stderr -s -k -u "$1" -L "${_h%/}/v2/token?account=${_userid}&client_id=docker&offline_token=true&service=${_h%/}/v2/token" | sed -E 's/.+"token":"([^"]+)".+/\1/')"
  echo "# Testing TOKEN:${_TOKEN} by requesting to ${_h%/}/v2/"
  curl -D/dev/stderr -k -H "Authorization: Bearer ${_TOKEN}" -H "Accept: application/json" -L "${_h%/}/v2/"
done
