...
function chk_sys() {
    local __doc__="Execute OS command to check system for triaging"
    local _work_dir="$1"	# case number as an ID
    local _u="$2"	# component process owner
    local _p="$3"	# Java PID

    if [ -z "${_work_dir}" ]; then
        _work_dir="$(date +"%Y%m%d%H%M%S")"
    fi

    if [ ! -d "$_work_dir" ]; then
        if ! mkdir $_work_dir; then
            echo "ERROR: Couldn't create $_work_dir directory"
            return 1
        fi
    fi
    ...
    wait
    tar czvf ./chksys_$(hostname)_$(date +"%Y%m%d%H%M%S").tar.gz ${_work_dir%/}/*.out
}
