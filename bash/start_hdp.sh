...
#
# Steps:
# 1. Install OS. Recommend Ubuntu 16.x, 18.x
...
function f_ambari_agent_reset() {
    ...
    if [ $? -eq 0 ]; then
        f_ambari_add_comp "${_agent_host}" "METRICS_MONITOR" "${_ambari_host}"
    fi
...
function f_ambari_add_comp() {
    local __doc__="Add (client) component as Ambari Web UI installs all clients (f_ambari_add_comp node1.localdomain HDFS_CLIENT"
    local _host="$1"
    local _comp="$2"
    local _ambari_host="${3-$r_AMBARI_HOST}"
    local _c="`f_get_cluster_name ${_ambari_host}`" || return 1
...
function p_host_setup() {
    ...
    p_basic_setup &>> /tmp/p_host_setup.log || return $?
    ...
    if ! _isYes "$r_AMBARI_NOT_INSTALL"; then
        ...
        if _isYes "$r_AMBARI_BLUEPRINT"; then
            ...
            _log "INFO" "*Scheduling* f_ambari_performance"
            echo "bash `realpath $BASH_SOURCE` -r `realpath ${g_RESPONSE_FILEPATH}` -f f_ambari_performance" | at now +1 hour
        else
...
if [ "$0" = "$BASH_SOURCE" ]; then
    ...
    _info "You may want to source ${g_SETUP_HOST}"
fi
