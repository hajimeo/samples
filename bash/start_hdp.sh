...
function f_ambari_server_install() {
    ...
    ssh root@$r_AMBARI_HOST "yum install ambari-server -y && ambari-server setup -s || ( echo 'ERROR ambari-server setup failed! Trying one more time...'; for i in {1..3}; do [ -e /tmp/.s.PGSQL.5432 ] && break; sleep 5; done; sed -i.bak '/server.jdbc.database/d' /etc/ambari-server/conf/ambari.properties; ambari-server setup -s --verbose )"
}
