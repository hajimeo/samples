...
If you would like to fis this now, press Ctrl+c to stop (sleep 7 seconds)"
    ...
    if ! nc -z ${_HOSTNAME} ${_AMBARI_PORT}; then
        docker exec -it ${_NAME} bash -c 'service ambari-server start --skip-database-check || (service postgresql start;sleep 5;service ambari-server restart --skip-database-check)'
    fi
