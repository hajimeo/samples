...
usage() {
    cat << 'EOF'
PURPOSE:
    Delete one or multiple blobRefs (call this script concurrently for many blobRefs)
...
OPTIONS:
    ...
    -b  blobRefs (comma separated), or a file contains lines of blobRefs
EOF
}
...
: "${_USE_SED:="true"}"   # If the blobRefs file contains extra strings, use sed to extract valid blobRefs
: "${_TMP:="/tmp"}"
_SCRIPT_NAME="deleteByBlobRefs"
...
main() {
    ...
    # If _blobRefs is a file, read the file and convert to comma separated string
    if [ -s "${_blobRefs}" ]; then
        ...
        if [ "${_USE_SED}" == "true" ] && type sed >/dev/null 2>&1; then
            ...
            _blobRefs="${_TMP%/}/blobRefs_$$.tmp"
        fi
        if type xargs >/dev/null 2>&1; then
            cat << EOF > "${_TMP%/}/${_SCRIPT_NAME}_batch.sh"
#!/usr/bin/env bash
_blobRefs="\$(echo "\$@" | tr " " ",")"
curl -sSf -L -k -u '${_ADMIN_USER}:${_ADMIN_PWD}' -H 'Content-Type: application/json' "${_NEXUS_URL%/}/service/rest/v1/script/${_SCRIPT_NAME}/run" -d"{\"blobRefs\":\"\${_blobRefs%,}\",\"blobStore\":\"${_blobStore}\",\"noBsChk\":${_NO_BS_CHK:-"false"},\"dryRun\":${_DRY_RUN:-"false"},\"debug\":${_DEBUG:-"false"}}"
echo ""
EOF
            if [ "${_DEBUG}" == "true" ]; then
                echo "Created ${_TMP%/}/${_SCRIPT_NAME}_batch.sh with content:"
                cat "${_TMP%/}/${_SCRIPT_NAME}_batch.sh"
                cat "${_blobRefs}" | xargs -P ${_PARALLEL:-1} -L ${_BATCH_SIZE:-1} -t bash -x ${_TMP%/}/${_SCRIPT_NAME}_batch.sh
            else
                cat "${_blobRefs}" | xargs -P ${_PARALLEL:-1} -L ${_BATCH_SIZE:-1} bash ${_TMP%/}/${_SCRIPT_NAME}_batch.sh
            fi
            # TODO: xargs do not stop at the first error
            return $?
        fi
    fi
    curl -sSf -L -k -u "${_ADMIN_USER}:${_ADMIN_PWD}" -H 'Content-Type: application/json' "${_NEXUS_URL%/}/service/rest/v1/script/${_SCRIPT_NAME}/run" -d'{"blobRefs":"'${_blobRefs%,}'","blobStore":"'${_blobStore}'","dryRun":'${_DRY_RUN:-"false"}',"debug":'${_DEBUG:-"false"}'}'
}
