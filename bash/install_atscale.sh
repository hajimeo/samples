...
function f_pg_ctl() {
    ...
    local _lib_path="$(ls -1dtr ${_pg_dir%/}/lib | head -n1)"
    sudo -u ${_ATSCALE_USER} LD_LIBRARY_PATH=${_lib_path} PGPASSWORD=${PGPASSWORD:-atscale} ${_pg_dir%/}/bin/pg_ctl -D ${_pg_dir%/}/data "$@"
}
...
function f_pg_restore() {
    ...
    #LD_LIBRARY_PATH=${_lib_path} PGPASSWORD=${PGPASSWORD:-atscale} ${_pg_dir%/}/bin/pg_restore -h localhost -p 10520 -U atscale -v -C -d atscale $_dump_file
...
function f_psql() {
    ...
    local _pg_dir="${_ATSCALE_DIR%/}/share/postgresql-9.*/"
    local _lib_path="$(ls -1dtr ${_pg_dir%/}/lib | head -n1)"
    LD_LIBRARY_PATH=${_lib_path} PGPASSWORD=${PGPASSWORD:-atscale} ${_pg_dir%/}/bin/psql -h localhost -p 10520 -U atscale "$@"
}
