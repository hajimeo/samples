...
function f_check_system() {
    ...
    dmesg --time-format iso | grep 'link up' &> ${_work_dir%/}/dmesg_link_up.out
...
function f_collect_log_files() {
    ...
    # grep -i "killed process" /var/log/messages* # TODO: should we also check OOM Killer? (also fs.inotify.max_user_watches)
}
