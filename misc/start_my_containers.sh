...
main() {
    ...
    for n in node-nxiq node-nxrm-ha1; do
        bash /usr/local/bin/setup_standalone.sh -N -n $n &>/tmp/setup_standalone_$n.out #|| return 1
        sleep 1
    done
    ...
    #done
    #wait
    bash /usr/local/bin/setup_standalone.sh -N -n nexus-client &>/tmp/setup_standalone_nexus-client.out
    sleep 60
