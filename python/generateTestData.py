...
def randomStr(length, prefix=""):
    return prefix + ''.join(random.choice(string.ascii_lowercase) for i in range(length))
...
def tmpCaseDocumentationQualityCheck(csv_file_path, case_owner_column='Case Owner', how_many=4, dialect='excel'):
    ...
    for row in r:
        ...
        if len(row) < (case_owner_index + 1):
            log.debug("line number %s does not have case owner column(%s). Skipping..." % (
            str(r.line_num), str(case_owner_index)))
            continue
        if len(row[case_owner_index]) < 1:
            log.debug("line number %s does not have case owner column(%s). Skipping.." % (
            str(r.line_num), str(case_owner_index)))
