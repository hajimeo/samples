...
== DuckDB tips (which I often forget) ==================================
...
Get date_hour
  ...
  convert request.log's date to time:
    substr(date, 13, 8)::TIME as hhmmss
Format datetime to request.log like one (No month abbreviation without UDF_STRFTIME):
    UDF_STRFTIME('%d/%b/%Y:%H:%M:%S', date_time - INTERVAL 30 SECOND)||' +0000' as req_date_time
Convert current time or string date to Unix timestamp
    epoch(NOW())
    epoch(UDF_REGEX('(\d\d\d\d-\d\d-\d\d.\d\d:\d\d:\d\d.\d+)', max(date_time), 1)::TIMESTAMP)
  or
    UDF_TIMESTAMP('some date like string')
  or
    $ q "select epoch('2020-05-01 00:10:00'::TIMESTAMP)"
    1588291800
...
Get request started time by concatenating today and the time string from 'date' column, then convert to Unix-timestamp
    (epoch(CAST(DATE('now')||' '||substr(date,13,8) AS TIMESTAMP)) - elapsedTime/1000)::BIGINT
...
import sys, os, io, fnmatch, gzip, re, json, ast, tempfile, linecache
import duckdb

try:
    from duckdb.typing import VARCHAR, BIGINT, DOUBLE
except ImportError:  # newer duckdb renamed this module to duckdb.sqltypes
...
def _db(conn_str=_DB_CONN_STR, dbtype='duckdb', isolation_level=None, use_sqlalchemy=False, echo=False):
    """
    Create a DB object. For performance purpose, currently not using sqlalchemy if dbtype is duckdb
    ...
    global _DB_TYPE
    _DB_TYPE = dbtype
    if use_sqlalchemy is False and dbtype == 'duckdb':
        return duckdb.connect(database=conn_str)
    if dbtype == 'sqlite':
...
# DuckDB has native regexp_extract/regexp_matches, but this UDF keeps naming/behavior compatible with older sqlite-based notebooks
def udf_regex(regex, item, rtn_idx=0):
    ...
    :return:        String  - Group(idx) result, or 'True'/'False' when rtn_idx is 0
    >>> udf_regex('(\d\d\d\d-\d\d-\d\d.\d\d)', "2019-10-14 00:00:05", 1)
    '2019-10-14 00'
    """
    matches = re.search(regex, item)
    # If 0, return 'True'/'False' string (DuckDB UDFs need one fixed return type; cast with ::BOOLEAN if used in WHERE)
    if rtn_idx == 0:
...
def _register_udfs(conn):
    ...
    How to check: SELECT function_name FROM duckdb_functions() WHERE function_name like 'udf_%';
    :param conn:
    :return:
    """
    global _LOAD_UDFS
    if _LOAD_UDFS:
        ...
        conn.create_function("UDF_REGEX", udf_regex, [VARCHAR, VARCHAR, BIGINT], VARCHAR)
        conn.create_function("UDF_COUNT_CHAR", udf_count_char, [VARCHAR, VARCHAR], BIGINT)
        conn.create_function("UDF_STR2SQLDT", udf_str2sqldt, [VARCHAR], VARCHAR)
        conn.create_function("UDF_STRFTIME", udf_strftime, [VARCHAR, VARCHAR], VARCHAR)
        conn.create_function("UDF_TIMESTAMP", udf_timestamp, [VARCHAR], BIGINT)
        conn.create_function("UDF_TIMESTAMP_MS", udf_timestamp_ms, [VARCHAR], BIGINT)
        conn.create_function("UDF_TIMESTAMP_TO_DATE", udf_timestamp_to_date, [VARCHAR], VARCHAR)
        conn.create_function("UDF_STARTED_TIME", udf_started_time, [VARCHAR, VARCHAR], VARCHAR)
        conn.create_function("UDF_STR_TO_INT", udf_str_to_int, [VARCHAR], DOUBLE)
        conn.create_function("UDF_NUM_HUMAN_READABLE", udf_num_human_readable, [VARCHAR, VARCHAR], VARCHAR)
    return conn


def connect(conn_str=_DB_CONN_STR, dbtype='duckdb', isolation_level=None, use_sqlalchemy=False, echo=False):
    """
    Connect to a database (DuckDB)
    :param conn_str: Database name
    :param dbtype: DB type duckdb or postgres
    :param isolation_level: Isolation level
    :param use_sqlalchemy: Use sqlalchemy
    :param echo: True outputs more if sqlalchemy is used
    :return: connection (cursor) object
    >>> import duckdb;s = connect()
    >>> isinstance(s, duckdb.DuckDBPyConnection)
    True
    """
    global _LAST_CONN
    ...
    db = _db(conn_str=conn_str, dbtype=dbtype, isolation_level=isolation_level, use_sqlalchemy=use_sqlalchemy,
             echo=echo)
    if dbtype == 'duckdb':
        # For 'duckdb, 'db' is the connection object because of _db()
...
def execute(sql, conn=None, no_history=False):
    ...
    >>> _ = execute("CREATE TEMP TABLE IF NOT EXISTS testtesttesttest(id INT)", connect(), True)
    >>> r = execute("UPDATE testtesttesttest SET id = id WHERE 1 = 0", connect(), True)
    >>> bool(r)
    True
    >>> _ = execute("DROP TABLE testtesttesttest", connect(), True)
    """
    if conn is None:
        conn = connect()
...
def query(sql, conn=None, no_history=False, show=False):
    """
    Run given query natively against DuckDB, expecting SELECT statement
    :param sql: SELECT statement
    :param conn: DB connection object
    :param no_history: not saving this query into a history file
    :param show: True/False or integer to draw HTML (NOTE: False is faster)
    :return: a DF object or void
    >>> query("select table_name as name from duckdb_tables()", connect(), True)
    Empty DataFrame
    Columns: [name]
    Index: []
    """
...
def draw(df, width=16, x_col=0, x_colname=None, name=None, desc="", tail=10, is_x_col_datetime=True, kind="line",
    ...
    if group_col:
        for key, grp_df in df.groupby([group_col]):
            plot(grp_df, width=width, height_inch=height_inch, x_colname=x_colname, name=name + " => " + str(key),
                 kind=kind)
    else:
...
def describe(tablename=None, colname=None, conn=None):
    ...
    if _DB_TYPE == 'duckdb':
        ...
        sql = "SELECT column_name AS name, data_type AS type FROM information_schema.columns WHERE table_name = '%s' %s ORDER BY ordinal_position" % (
            tablename, extra_where)
        return query(sql=sql, conn=conn, no_history=True)

    if _DB_TYPE != 'sqlite':
        # If not sqlite/duckdb, expecting information_schema is available
        extra_where = ""
        if bool(colname):
            extra_where = " AND column_name like '" + colname + "%'"
        if bool(tablename) is False:
...
def show_create_table(tablenames=None, like=None, conn=None):
    ...
    """
    global _DB_TYPE
    if _DB_TYPE not in ('sqlite', 'duckdb'):
    ...
    if _DB_TYPE == 'duckdb':
        if bool(tablenames):
            ...
            for t in tablenames:
                rs = execute("select sql from duckdb_tables() where table_name = '%s'%s" % (str(t), sql_and))
                if bool(rs) is False:
                    continue
                rows = rs.fetchall()
                if not rows:
                    continue
                print(rows[0][0])
                print("Rows: %s\n" % (execute("SELECT count(*) FROM \"%s\"" % (t)).fetchall()[0][0]))
            return
        if bool(like):
            rs = execute("select distinct table_name from duckdb_tables() where 1=1%s" % (sql_and))
            if bool(rs) is False:
                return
            tablenames = _get_col_vals(rs.fetchall(), 0)
            return show_create_table(tablenames=tablenames)
        return query(
            sql="select distinct table_name as name from duckdb_tables() where 1=1%s order by table_name" % (sql_and),
...
def df2table(df, tablename, conn=None, chunksize=1000, if_exists='replace', schema=None):
    ...
    global _DB_TYPE
    if _DB_TYPE == 'duckdb':
        conn.register('_df2table_tmp', df)
        try:
            if if_exists == 'replace':
                conn.execute('CREATE OR REPLACE TABLE "%s" AS SELECT * FROM _df2table_tmp' % (tablename))
            elif if_exists == 'append':
                if not exists(tablename, conn=conn):
                    conn.execute('CREATE TABLE "%s" AS SELECT * FROM _df2table_tmp' % (tablename))
                else:
                    conn.execute('INSERT INTO "%s" SELECT * FROM _df2table_tmp' % (tablename))
            else:  # 'fail'
                if exists(tablename, conn=conn):
                    _info("%s already exists" % (tablename))
                    return False
                conn.execute('CREATE TABLE "%s" AS SELECT * FROM _df2table_tmp' % (tablename))
    ...
    try:
        ...
        _err("NOT importable into %s due to: %s" % (tablename, str(e)))
        return False
    return True
...
def etl(path="", log_suffix=".log", dist="./_filtered", max_file_size=(1024 * 1024 * 100), time_from_regex=None,
    ...
    if bool(path) is False:
        maybe_zips = _globr("*support*.zip", depth=1)
        if len(maybe_zips) > 0:
            path = maybe_zips[-1:][0]
            _info("'path' is not specified and found zip file: %s . Using this one..." % path)
    ...
    try:
        ...
        _system(
            _SH_EXECUTABLE + " -c '[ ! -s /tmp/log_search.sh ] && curl -s --compressed https://raw.githubusercontent.com/hajimeo/samples/master/bash/log_search.sh -o /tmp/log_search.sh; [ ! -d \"%s\" ] && mkdir \"%s\"'" % (
                dist, dist))
        _system(  # f_request2csv \"\" \"%s\" 2>/dev/null &&
            _SH_EXECUTABLE + " -c '%s[ -d \"%s\" ] && . /tmp/log_search.sh && f_audit2json \"\" \"%s\"'" % (
        ...
        _ = load_jsons(src=dist, include_ptn="db_*.json", flatten=True, max_file_size=(max_file_size * 2),
                       conn=connect())
        ...
        if os.path.isfile(
            ...
            _ = json2df(dist + "/audit.json", tablename="t_audit_logs", flatten=True, max_file_size=max_file_size)
        else:
            # TODO: currently below is too slow, so not using "max_file_size=max_file_size,"
            log_path = _get_file("audit" + log_suffix)
            if bool(log_path):
                line_from = line_until = 0
                if bool(time_from_regex):
                    line_from = _linenumber(log_path, "^\{\"timestamp\":\"\d\d\d\d-\d\d-\d\d " + time_from_regex)
                if bool(time_until_regex):
                    line_until = _linenumber(log_path, "^\{\"timestamp\":\"\d\d\d\d-\d\d-\d\d " + time_until_regex)
                _ = json2df(log_path, tablename="t_audit_logs",
                            line_by_line=True, line_from=line_from, line_until=line_until)

        # ./work/db/xxxxx.json (new DB)
        _ = load_jsons(".", include_ptn=".+/db/.+\.json", exclude_ptn='(samlConfigurationExport\.json|export\.json)',
                       useRegex=True, conn=connect())

        # xxxxx.csv. If CSV, probably 3 times higher should be OK
        _ = load_csvs(src="./_filtered/", include_ptn="*.csv", max_file_size=(max_file_size * 5), conn=connect())
        ...
        # Thread dump
        _ = logs2table(filename="threads.txt", tablename="t_threads", conn=connect(),
...
def etl_request(log_suffix=".log", max_file_size=(1024 * 1024 * 100), add_startTime=True, tablename="t_request"):
    if exists(tablename) is False and os.path.exists("./_filtered/request.csv"):
        # If request.*csv* exists, use that. if not, logs2table, which is slower.
        _ = load_csvs(src="./_filtered/", include_ptn="request.csv", max_file_size=(max_file_size * 5),
                      conn=connect())
    # If no csv file, then try to load request.log
    if exists(tablename) is False:
        _ = request2table("request" + log_suffix)
    if exists(tablename):
        _info("Executing req2table_post for %s with add_startTime %s" % (tablename, add_startTime))
        req2table_post(tablename=tablename, add_startTime=add_startTime)
        global isHeaderContentLength
        # non NXRM3 request.log wouldn't have headerContentLength
        isHeaderContentLength = exists(tablename, "headerContentLength")
        return True
    return False
...
def req2table_post(tablename="t_request", add_startTime=True, datetime_col="date", elapsed_col="elapsedTime"):
    ...
    :return: void
    """
    if exists(tablename, "headerContentLength"):
        try:
            _info("Changing '-' in headerContentLength to 0 ...")
            _ = execute(sql=f"UPDATE {tablename} SET headerContentLength = 0 WHERE headerContentLength = '-'")
        except:
            pass
    if add_startTime:
        try:
            _ = execute(sql=f"ALTER TABLE {tablename} ADD COLUMN startTime timestamp DEFAULT NULL")
        except Exception as e:
            _err(str(e))
        try:
            _ = execute(sql=f"ALTER TABLE {tablename} ADD COLUMN endTime timestamp DEFAULT NULL")
        except Exception as e:
            _err(str(e))
        try:
            _info(f"Populating endTime from '{datetime_col}' column ...")
            _ = execute(sql=f"UPDATE {tablename} SET endTime = UDF_STRFTIME('%Y-%m-%d %H:%M:%S', `{datetime_col}`)")
        except Exception as e:
            _err(str(e))
        try:
            _info(f"Populating startTime from '{datetime_col}' and '{elapsed_col}' columns (this one is slow) ...")
            _ = execute(sql=f"UPDATE {tablename} SET startTime = UDF_STARTED_TIME(`{datetime_col}`, {elapsed_col})")
        except Exception as e:
            _err(str(e))
...
def request2table(filepath, tablename="t_request", max_file_size=(1024 * 1024 * 100), time_from_regex=None,
    ...
    """
    log_path = _get_file(filepath)
    if bool(log_path) is False:
        return False
    line_from = line_until = 0
    if bool(time_from_regex):
        line_from = _linenumber(log_path, r"\d\d/.../\d\d\d\d:" + time_from_regex)
    if bool(time_until_regex):
        line_until = _linenumber(log_path, r"\d\d/.../\d\d\d\d:" + time_until_regex)
    (col_names, line_matching) = _gen_regex_for_request_logs(log_path)
    rtn = logs2table(log_path, tablename=tablename, line_beginning="^.",
                     col_names=col_names, line_matching=line_matching,
                     max_file_size=max_file_size,
                     line_from=line_from, line_until=line_until)
    if exists(tablename):
        _info("%s has been created (may want to run al.req2table_post('%s'))" % (tablename, tablename))
        return True
    return False
...
def applog2table(filepath, tablename="t_applog", max_file_size=(1024 * 1024 * 100), time_from_regex=None,
    ...
    line_from = line_until = 0
    if bool(time_from_regex):
        line_from = _linenumber(log_path, r"^\d\d\d\d-\d\d-\d\d " + time_from_regex)
    if bool(time_until_regex):
        line_until = _linenumber(log_path, r"^\d\d\d\d-\d\d-\d\d " + time_until_regex)
    (col_names, line_matching) = _gen_regex_for_app_logs(log_path)
    return logs2table(log_path, tablename=tablename, col_names=col_names,
...
def _gen_regex_for_request_logs(filepath="request.log"):
    ...
    _info("Can not determine the log format for %s . Using default one." % (str(filepath)))
    return (columns, partern_str)
